...
import { CONFIG } from './js/config.js?v=1.3.0';
import { citationLabels, detectQuery, formatDate, paginateResults, dedupeResults } from './js/core.js?v=1.3.0';
import { searchDirectSources, searchGateway, SOURCE_CATALOG } from './js/providers.js?v=1.3.0';
import { getSubject, SUBJECTS } from './js/subjects.js?v=1.3.0';
...
function updateQueryKind() {
  ...
  elements.queryKind.textContent = value ? detectQuery(value).label : '';
}
...
function inferArea(result) {
  if (state.subjectId !== 'all') return state.subjectId;
...
function authorLine(result) {
  ...
  const short = `${authors.slice(0, 8).join(', ')} + ${authors.length - 8} more`;
  paragraph.append(document.createTextNode(short + ' '));
  const button = element('button', { className: 'author-toggle', type: 'button', text: 'Show all' });
  let expanded = false;
  button.addEventListener('click', () => {
    expanded = !expanded;
    paragraph.firstChild.textContent = `${expanded ? authors.join(', ') : short} `;
...
function updateUrl(query, subjectId) {
  const url = new URL(window.location.href);
  url.searchParams.set('q', query);
  if (subjectId === 'all') url.searchParams.delete('area');
  else url.searchParams.set('area', subjectId);
  history.replaceState(null, '', url);
}

async function runSearch(overrides = {}) {
  const query = String(overrides.query ?? elements.query.value).trim();
  const subjectId = overrides.subjectId ?? elements.subject.value;
  if (!query) {
  ...
  elements.subject.value = subjectId;
  updateQueryKind();
  setLoading(true);
  ...
  elements.resultSummary.textContent = `${getSubject(subjectId).label} · live scholarly records`;
  elements.sourceSummary.hidden = true;
  ...
  updateUrl(query, subjectId);

  try {
    let data;
    if (CONFIG.apiBase) {
      try {
        data = await searchGateway({ apiBase: CONFIG.apiBase, query, subjectId, signal: controller.signal, timeoutMs: CONFIG.requestTimeoutMs, limit: CONFIG.resultsPerSource });
      } catch (gatewayError) {
        if (controller.signal.aborted) throw gatewayError;
        data = await searchDirectSources({ query, subjectId, signal: controller.signal, timeoutMs: CONFIG.requestTimeoutMs, limit: CONFIG.resultsPerSource });
    ...
    elements.resultsEyebrow.textContent = detectQuery(query).label.toUpperCase();
    state.page = 1;
    ...
    elements.resultSummary.textContent = `${getSubject(subjectId).label} · merged and ranked · retrieved ${retrieved}`;
    ...
    elements.resultSummary.textContent = `${getSubject(subjectId).label} · query preserved`;
...
async function loadMore() {
  ...
  try {
    const data = await searchDirectSources({ query: state.query, subjectId: state.subjectId,
...
elements.modeBadge.textContent = CONFIG.apiBase ? 'Gateway mode · up to 6 sources' : 'Direct mode · 2 live sources';
    ...
    await context.registerTool({
      ...
      inputSchema: {
        ...
        properties: {
          ...
          subject: { type: 'string', enum: SUBJECTS.map((subject) => subject.id), description: 'Physics subject area ID. Defaults to all.' },
        },
        required: ['query'],
      ...
      async execute(input) {
        ...
        const subjectId = SUBJECTS.some((subject) => subject.id === input.subject) ? input.subject : 'all';
        const data = await runSearch({ query: input.query, subjectId });
...
if (initialQuery) {
  const subjectId = SUBJECTS.some((subject) => subject.id === initialArea) ? initialArea : 'all';
  elements.query.value = initialQuery;
  elements.subject.value = subjectId;
  updateQueryKind();
  void runSearch({ query: initialQuery, subjectId });
}
