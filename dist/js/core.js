...
// Matches initials as prefixes while requiring every supplied name part to
// belong to one author. This prevents "Albert Einstein" from matching a paper
// merely because Albert and Einstein occur in different author names.
export function authorNameMatches(query = '', candidate = '') {
  const wanted = nameTokens(query);
  const available = nameTokens(candidate);
  if (!wanted.length || !available.length) return false;
  if (wanted.length === 1) return available.some((part) => part === wanted[0]);
