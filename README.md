...
The static site works immediately with Crossref and OpenAlex. An optional Cloudflare Worker adds a CORS-safe aggregation layer for arXiv, Semantic Scholar, INSPIRE-HEP, and NASA ADS, while keeping API keys out of the browser.
...
- Eleven physics subject groups based on arXiv's taxonomy and mapped to other providers where they support comparable filters.
...
dist/                  Static website deployed by GitHub Pages
  ...
  js/
    ...
    providers.js       Direct OpenAlex and Crossref adapters
...
In the default direct mode, the page queries OpenAlex and Crossref from the visitor's browser. If a browser, network policy, rate limit, or upstream CORS policy blocks one API, the interface reports partial coverage instead of claiming that source was searched successfully.
...
```http
GET /api/search?q=quantum+error+correction&area=quantum&limit=15
GET /api/health
```

`area` is one of `all`, `astro`, `condmat`, `hep`, `grav`, `quantum`, `nuclear`, `amo`, `stat`, `math`, `plasma`, or `applied`. `limit` is per source and capped at 30. Responses are cached at the edge for 15 minutes, with shorter browser caching.
...
| [INSPIRE-HEP](https://github.com/inspirehep/rest-api-doc) | No | Yes | None for reads |
...
Exact identifiers only query sources with an implemented lookup route. Direct mode supports DOI and OpenAlex IDs; arXiv IDs, PMID, CorpusId and ADS bibcodes require the optional gateway and an appropriate configured source. Unsupported sources are marked “Skipped”, rather than sending the identifier as an unrelated text query. An exact lookup returning HTTP 404 is treated as no match, not a service outage. Questions use OpenAlex semantic search when a gateway key is configured and lexical search elsewhere.

Subject filtering is approximate: OpenAlex uses broad subfields, arXiv and INSPIRE use category mappings, and Crossref, Semantic Scholar and ADS do not implement the selected fine-grained area filter. Exact identifiers bypass subject restrictions. A selected area is not presented as verified subject metadata for a result.
...
- **Only two sources are searched:** `apiBase` is empty or the Worker is unreachable. Deploy the gateway and set its public origin in `dist/js/config.js`.
...
- A search sends the query and selected area to the configured gateway, or directly to the two sources named in direct mode. Upstream providers may log requests under their own policies.
