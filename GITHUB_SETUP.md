...
The default website searches Crossref and OpenAlex directly. It includes entries-per-page controls, Load more, citation-source labels, and OpenAlex citing-paper links. It needs no backend, API key, build command, or ChatGPT account. Availability still depends on the providers' current access policies, limits, and browser CORS support. The optional backend adds the other sources where access permits; NASA ADS needs a token.
...
- **Only two sources are live:** that is expected in the default mode. The other sources need the optional backend; NASA ADS also needs its token.
